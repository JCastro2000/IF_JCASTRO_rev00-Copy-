...
\caption[Sistema de ejes locales de la pala.]{Sistema de ejes locales de la pala sobre la sección del perfil. El eje $X_l$ es normal a la cuerda y positivo hacia el eje del rotor, $Y_l$ sigue la cuerda y es positivo en el sentido del movimiento de la pala, y $Z_l$ recorre la envergadura hacia arriba. Los sentidos positivos corresponden a los que se determinan por medición en la Sección~\ref{sec:ejes_locales_res}.}
...
Los brazos de soporte se incorporan a la definición de la turbina con cinco paneles aerodinámicos cada uno y distribución de tipo coseno, de modo que su arrastre interviene en el balance de par del rotor. Su sustentación, en cambio, se mantiene desactivada. La razón es que QBlade la resuelve asignando un perfil aerodinámico al brazo, y no se dispone de la sección real de estos elementos, de modo que activarla obligaría a suponer una geometría no documentada. Una comprobación específica sobre el caso de $12$~m/s cuantifica el alcance de ese supuesto, ya que habilitar la sustentación de los brazos con una sección supuesta reduce el coeficiente de potencia en un $2{,}6\%$, magnitud que acota el efecto sin resolverlo. La exclusión se declara, por tanto, como una limitación del modelo, que sobreestima ligeramente la potencia en esa proporción y cuyo efecto sobre la carga de la pala no queda acotado por esa misma medida.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        Distancia de truncamiento & $15$ & diámetros de rotor \\
        Wake Reduction Factor & $0{,}02$ & — \\
        Wake Zones (N/1/2/3), anchos & $1$ / $2$ / $4$ / $5$ & revoluciones del rotor \\
        Refinamiento Streamwise (1/2/3) & $2$ / $2$ / $2$ & — \\
        Refinamiento Spanwise (1/2/3) & $1$ / $1$ / $2$ & — \\
        \hline
...
\medskip
\noindent\textbf{Integración de largo plazo sobre la distribución del emplazamiento.}
La probabilidad anterior está condicionada a una velocidad fija. Dado que cada velocidad $V_j$ ocurre con una frecuencia distinta a lo largo del año, se requiere la probabilidad de excedencia no condicionada. Para obtenerla, la norma prescribe integrar sobre la distribución de velocidades de viento en el emplazamiento (Ecuación G.2, Sección G.3.2 del Anexo G de la norma IEC 61400-1)~\parencite{IEC61400-1}.

Como ponderación se emplea la frecuencia empírica de la propia serie del emplazamiento, contada banda a banda sobre los $38$ años de registro extrapolados a la altura del centro del rotor (Sección~\ref{sec:recurso_metodo}), en lugar de la densidad de Weibull ajustada a esa misma serie. La Sección G.3.4 de la norma, al describir la agregación de datos de todas las velocidades, admite expresamente ponderar por la distribución de Weibull o por la que resulte apropiada~\parencite{IEC61400-1}, y aquí la apropiada es la empírica. El motivo es que el ajuste de Weibull, aun reproduciendo bien el grueso de la distribución, subestima la banda alta de velocidades, que es precisamente la que domina la cola de la extrapolación, de modo que emplearlo introduciría un sesgo en contra del margen de seguridad. La magnitud de esa desviación se cuantifica en el Capítulo~4. Los parámetros $k$ y $c$ se conservan como caracterización sintética del recurso y siguen empleándose en el cálculo de fatiga, donde la ponderación abarca todo el rango y no solo su extremo.

Numéricamente, la integral se discretiza en bandas de ancho $\Delta V = 1$~m/s centradas en $V = 2{,}5; 3{,}5; \dots; 24{,}5$~m/s, esto es, $N_V = 23$ bandas que cubren el rango operacional. Las velocidades por debajo de $V_{in}$ y por encima de $V_{out}$ quedan fuera, ya que en ellas la turbina no produce y sus cargas corresponden a los casos de turbina detenida. Como solo se dispone de ajustes de Gumbel en las $12$ velocidades simuladas, los parámetros $\mu(V)$ y $\beta(V)$ se interpolan linealmente sobre las bandas. La probabilidad de excedencia de largo plazo resulta según la Ec.~\ref{eq:pe_longterm}:

\begin{equation}
    P_e(F_{max}) = \sum_{m=1}^{N_V} \Bigl[1 - \exp\!\bigl(-e^{-(F_{max} - \mu(V_m))/\beta(V_m)}\bigr)\Bigr] \cdot p_m
    \label{eq:pe_longterm}
\end{equation}

donde $p_m$ es la fracción de registros de la serie que caen en la banda $m$.

En esta expresión cada velocidad se pondera por su frecuencia de ocurrencia anual, de modo que las cercanas a $V_{ave}$ reciben mayor peso a pesar de producir fuerzas moderadas, mientras que las próximas a $V_{out}$ reciben menor peso aunque generen fuerzas mayores. La combinación de ambos efectos, frecuencia de ocurrencia y magnitud de la Gumbel, determina la contribución relativa de cada velocidad al riesgo de excedencia a largo plazo.
